...
export default function PopupModal({
  ...
  const submitUserData = e => {
    ...
    const URL =
      type === "Toggle Cohort"
        ? `/api/toggleCohort/${data.class_id}`
        : type === "updating"
        ? `/api/updateTitleDesc/${data.class_id}`
        : type === "approving"
        ? `/api/toapprove/${data.user_id}`
        : type === "disapproving"
        ? `/api/todisapprove/${data.user_id}`
        : type === "Create Cohort"
        ? `/api/class`
        : type === "Change Key"
        ? `/api/class/${data.classroom_id}`
        : type === "Change User Role"
        ? `/api/assigning/${data.user_id}`
        : type === "Kick Student"
        ? `/api/kickstud/${data.user_id}/${data.class_id}`
        : null;
    axios({
      method: METHOD,
      url: URL,
      ...
      .then(() => {
        ...
        if (type === "approving" || type === "disapproving")
          socket.emit("handleRoleRequest", {
            user_id: data.user_id,
            approval_status: body.data
          });
        ...
        type === "Toggle Cohort" ? (
          <>
            ...
            <DialogActions>
              {type === "Change User Role" ? (
                <Button
                  onClick={e => setAttending({ open: true, data: data })}
                  color="secondary"
                  style={{ float: "right" }}
                  autoFocus
                >
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment
                          style={{
                            color:
                              counter.updatingDescription < 10 ? "red" : null,
                            opacity: "0.5"
                          }}
                          position="end"
                        >
                          {counter.updatingDescription}/100
                        </InputAdornment>
                      )
                    }}
                    inputProps={{
