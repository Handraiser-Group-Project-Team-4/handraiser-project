...
module.exports = {
    ...
    request: (req, res) => {
        ...
        db.query(`UPDATE users set user_approval_status_id=2 WHERE user_id = '${req.params.id}'`)
            .then(get => res.status(200).json(get))
            .catch(err => {
                console.error(err);
                res.status(500).end();
    ...
    user_approval_fetch: (req, res) => {
        ...
        db.query(`select * from users where user_approval_status_id = ${user_approval_status_id}`)
            .then(get => res.status(200).json(get))
            .catch(err => {
                console.error(err);
                res.status(500).end();
